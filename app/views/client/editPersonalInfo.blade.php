...
<form method="POST" action="{{$formUrl}}" id="editPersonalInfo">
    @if(UserHasRole::where('user_id', Auth::user()->id)->pluck('role_id') == 4)
        ...
        Business Nature : <input type="text" value="{{ $user->businessNature }}" name="businessNature" required="required"/><br/>
        Business Permit : <input type="text" value="{{ $user->businessPermit }}" name="businessPermit" required="required"/><br/>
